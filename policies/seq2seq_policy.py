...
class Seq2SeqPolicy():
    def __init__(self, obs_dim, encoder_units,
        ...
        self.obs = tf.placeholder(shape=[None, None, obs_dim], dtype=tf.float32)
        self.decoder_full_length = tf.placeholder(shape=[None], dtype=tf.int32)
        ...
        self.network = Seq2SeqNetwork( hparams = hparams, reuse=tf.AUTO_REUSE,
                 ...
                 decoder_targets=self.decoder_targets,name ="pi")
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        weights = policy.network.get_trainable_variables()

        tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, "pi")

        total_number = 0
        for var in weights:
            shape_value = list(var.shape)
            j = 1
            for i in shape_value:
                j *= i
            total_number += j

        print("total parameters are", total_number)