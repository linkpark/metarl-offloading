...
class PPOReptile():
    ...
    def UpdateMetaPolicy(self, task_samples, batch_size):
        # get the parameters value of the policy network
        sess = tf.compat.v1.get_default_session()

        for i in range(self.meta_batch_size):
            batch_number = int(task_samples[i]['observations'].shape[0] / batch_size)
            self.update_numbers = batch_number
            # observations = task_samples['observations']

            shift_actions = np.column_stack(
                (np.zeros(task_samples[i]['actions'].shape[0], dtype=np.int32), task_samples[i]['actions'][:, 0:-1]))

            observations_batchs = np.array(task_samples[i]['observations'])
            actions_batchs = np.array(task_samples[i]['actions'])
            shift_action_batchs = np.array(shift_actions)

            old_logits_batchs = np.array(task_samples[i]["logits"], dtype=np.float32)
            advs_batchs = np.array(task_samples[i]['advantages'], dtype=np.float32)
            oldvpred = np.array(task_samples[i]['values'], dtype=np.float32)
            returns = np.array(task_samples[i]['returns'], dtype=np.float32)

            sess = tf.compat.v1.get_default_session()

            decoder_full_length = np.array([observations_batchs.shape[1]] * observations_batchs.shape[0], dtype=np.int32)

            feed_dict = {self.old_logits[i]: old_logits_batchs, self.old_v[i]: oldvpred,
                         self.obs[i]: observations_batchs, self.actions[i]: actions_batchs,
                         self.decoder_inputs[i]: shift_action_batchs,
                         self.decoder_full_length[i]: decoder_full_length, self.advs[i]: advs_batchs,
                         self.r[i]: returns}

            grads = sess.run(self.inner_grads[i], feed_dict=feed_dict)
